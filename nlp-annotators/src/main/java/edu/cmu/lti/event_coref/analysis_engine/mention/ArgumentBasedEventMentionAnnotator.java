...
import edu.cmu.lti.event_coref.util.MappingDataReader;
...
/**
 * Annotated Event Mention using FrameNet annotations, could be abtained by
...
public class ArgumentBasedEventMentionAnnotator extends JCasAnnotator_ImplBase {
	...
	public void initialize(UimaContext context)
		...
		if (!eventFrameFile.exists()) {
			System.err
					.println("The given event frame name file does not exists");
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		if (goldStandardViewName != null) {
			try {
				...
				System.err
						.println("Gold standard view specified does not exists");
				e.printStackTrace();
			}
		}
		...
		for (SemaforAnnotationSet annoSet : JCasUtil.select(aJCas,
			...
			if (targetLabel != null) {
				...
				for (FanseToken token : JCasUtil.selectCovered(
					...
					System.out.println("Event token " + token.getCoveredText());
