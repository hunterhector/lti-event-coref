...
//    }
     ...
     * @return
     */
    public static boolean isTargetEventType(Logger logger, EventMention evm, String targetType) {
        String evmType = evm.getEventType();

        if (evmType == null) {
            if (targetType == null) {
                logger.info("Event type not initialized, now simply regard it as target event, it is suggested to check!");
                return true;
            } else
                return false;
        }
