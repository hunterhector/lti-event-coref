...
import edu.cmu.lti.event_coref.utils.EventCoreferenceConstants.EventType;
import edu.cmu.lti.event_coref.utils.SemanticRoleConstants.SemanticRole;
import edu.cmu.lti.utils.general.*;
...
//import edu.cmu.lti.event_coref.utils.stemming.StemmerUtils;

/**
 * This class provides useful utilsities specifically for event mentions.
 *
 * @author Jun Araki
 */
public class EventMentionUtils {

    private static final Logger logger = LoggerFactory.getLogger(EventMentionUtils.class);
    ...
    public static PairwiseEventCoreferenceEvaluation[][] getPairwiseGoldStandard(JCas aJCas,
                                                                                 EventCoreferenceRelationType corefType, List<EventMention> eventMentions,
        ...
        for (int i = 0; i < numOfEventMentions; i++) {
            for (int j = 0; j < numOfEventMentions; j++) {
                ...
                if (EventCoreferenceRelationType.FULL.equals(corefType)) {
                    // In the case of full coreference
                    if (EventCoreferenceClusterUtils.inSameEventCluster(eccList, evmI, evmJ)) {
                        pece.setEventCoreferenceRelationGoldStandard(EventCoreferenceRelationType.FULL
                                .toString());
                    } else {
    ...
    public static void writeErrorAnalysisReport(JCas aJCas, String outputFile,
        ...
        SourceDocumentInformation srcDocInfo = UimaJava5.getSingleton(aJCas,
    ...
    public static void printEventMentionDetail(EventMention evm) {
        logger.info(getEventMentionInfoWithComponentLinks(evm));
    }
    ...
    public static void printEventMentionDetail(EventMention evm, List<EntityBasedComponent> agents,
        ...
        logger.info(getEventMentionInfoWithComponentLinks(evm, agents, patients, locations));
    }
    ...
    public static boolean isTargetEventType(EventMention evm, String targetType) {
        ...
        if (evmType == null) {
            if (targetType == null) {
                logger.warn("Event type not initialized, now simply regard it as target event, it is suggested to check!");
                return true;
            } else
                return false;
        }
    ...
    public static boolean areSameListOfEventMentionAnnotations(EventMention evm1, EventMention evm2,
        ...
        if (evm1 == null && evm2 == null) {
            if (printWarning) {
                logger.warn("The both event mentions are null.");
            }
            return true;
        } else if (evm1 == null) {
            if (printWarning) {
                logger.warn("The first event mention is null whereas the second one isn't.");
            }
            return false;
        } else if (evm2 == null) {
            if (printWarning) {
                logger.warn("The second event mention is null whereas the first one isn't.");
            }
            return false;
        }
        ...
        if (begin1 != begin2) {
            if (printWarning) {
                logger.warn("The begin offsets of event mentions are different: begin1 " + begin1
                        + ", begin2 " + begin2);
            }
            return false;
        }
        ...
        if (end1 != end2) {
            if (printWarning) {
                logger.warn("The end offsets of event mentions are different: end1 " + end1
                        + ", end2 " + end2);
            }
            return false;
        }
        ...
        if (!evm1Str.equals(evm2Str)) {
            if (printWarning) {
                logger.warn("Strings of event mentions are different: evm1 [" + evm1Str + "], evm2 ["
                        + evm2Str + "]");
            }
            return false;
        }
    ...
    public static boolean areSameListOfEventMentionAnnotations(List<EventMention> evmList1,
        ...
        if (evmList1 == null && evmList2 == null) {
            if (printWarning) {
                logger.warn("The both lists of event mentions are null.");
            }
            return true;
        } else if (evmList1 == null) {
            if (printWarning) {
                logger.warn("The first list is null whereas the second one isn't.");
            }
            return false;
        } else if (evmList2 == null) {
            if (printWarning) {
                logger.warn("The second list is null whereas the first one isn't.");
            }
            return false;
        }
        ...
        if (evmList1Size != evmList2Size) {
            if (printWarning) {
                logger.warn("# of event mentions are different: # of first list " + evmList1Size
